...
var showError = function(error){
		...
		<main id="container" class="room_page sub_container">
			<div class="inner">
					...
					<div class="info_top">
						<div class="left">
							...
							<span class="addr">${ht.country } ${ht.city }
								${ht.province } ${ht.street }</span> <span class="grade">${grade }
								<em>/ 10</em>
							</span>
						</div>
						<div class="right">
							...
							<div class="btn_wrap">
								...
								<c:forEach items="${ roomLists }" var="row" varStatus="status">
									<c:if test="${status.index eq 0 }">
										<a id="room_idx" href="./order.do?hotel_idx=${row.room_idx }"
											class="site_go_btn">예약하기</a>
									</c:if>
								</c:forEach>
							</div>
						</div>
				...
				<div class="room_choice_wrap" id="room_choice_wrap">
					<input type="hidden" value="min" /> <input type="hidden"
						value="room_cnt" /> <strong class="room_tit">객실 선택</strong>
					<c:forEach items="${ roomLists }" var="row" varStatus="status">
									...
									<table class="room_tb">
										...
										<tbody>
											<tr>
												...
												<td class="pay_wrap">
													<div class="price">
														<strong class="price_txt"><fmt:formatNumber
																value="${p}" pattern="#,###" />원</strong> <span class="tax_txt">세금포함</span>
													</div>
													<div class="reserve_wrap">
														<a href="./order.do?hotel_idx=${row.room_idx }"
															class="reserve_btn">예약</a>
													</div>
												</td>
											</tr>
										</tbody>
				...
				<div id="map_wrap" class="map_wrap">
					<strong class="room_tit">지역 - ${ht.province }</strong> <span>${ht.city }
						${ht.province } ${ht.street }</span> <input type="hidden" id="latTxt"
						name="latTxt" value="${ht.ht_lat }" /> <input type="hidden"
						id="lngTxt" name="lngTxt" value="${ht.ht_lng }" />
					<div class="map">
	...
	$(document).ready(function() {
			...
			$('.reserve_btn').on('click', function(event) {
				...
				console.log("예약 버튼 클릭됨, 멤버 데이터: ", memberData);

				if (!memberData || !memberData.idx) {
					show();
				} else {
					const roomIdx = $(this).data('room-idx');
					window.location.href = './order.do?hotel_idx=' + roomIdx;
				}
			});
		});
	...
	<script async defer
		src="https://maps.googleapis.com/maps/api/js?key=AIzaSyA5-DB9e6-Lsqr4KB8zDoW5lxRmSffl4u8"></script>
</body>
</html>